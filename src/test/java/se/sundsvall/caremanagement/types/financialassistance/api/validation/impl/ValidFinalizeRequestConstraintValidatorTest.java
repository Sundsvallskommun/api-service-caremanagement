...
class ValidFinalizeRequestConstraintValidatorTest {
	...
	@Test
	void nonGrantingOutcomeWithoutAmountIsValid() {
	...
	void nullRequestOrMissingOutcomeIsLeftToTheFieldConstraints() {
		...
		// an unrecognised outcome is @OneOf's violation — no amount rule is applied on top of it
		assertThat(validator.isValid(FinalizeRequest.create().withDecision(FinalizeDecision.create().withOutcome("BEVILJAD")), contextMock)).isTrue();
		verifyNoInteractions(contextMock);
	}

	@Test
	void grantingOutcomeReportsMissingAmount() {
