...
class MessageHistoryPdfRendererTest {
	...
	void sanitisesUnencodableCharactersAndWrapsLongLines() throws IOException {
		...
		final var text = textOf(MessageHistoryPdfRenderer.renderMessages(ERRAND_NUMBER, thread, ThreadAttachments.flatten(thread)));
	...
	void rendersEmptyThread() throws IOException {
		final var pdf = MessageHistoryPdfRenderer.renderMessages(ERRAND_NUMBER, emptyList(), emptyList());
