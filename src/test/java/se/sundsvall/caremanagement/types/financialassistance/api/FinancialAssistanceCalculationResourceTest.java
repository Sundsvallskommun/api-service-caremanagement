...
class FinancialAssistanceCalculationResourceTest {
	...
	@Test
	void getDraft() {
		when(calculationServiceMock.getDraft(MUNICIPALITY_ID, NAMESPACE, ERRAND_ID))
			.thenReturn(CalculationDraft.create().withErrandId(ERRAND_ID)
		...
		final var response = webTestClient.get()
			.uri(uri -> uri.path(PATH + "/" + ERRAND_ID + "/calculation/draft").build(Map.of("municipalityId", MUNICIPALITY_ID, "namespace", NAMESPACE)))
			.exchange()
			.expectStatus().isOk()
		...
		verify(calculationServiceMock).getDraft(MUNICIPALITY_ID, NAMESPACE, ERRAND_ID);
	}

	@Test
	void patchDraftHeader() {
		when(calculationServiceMock.patchDraftHeader(eq(MUNICIPALITY_ID), eq(NAMESPACE), eq(ERRAND_ID), any(NormHeaderInput.class)))
			.thenReturn(CalculationDraft.create().withNormId(5).withHouseholdSize(1));

		final var response = webTestClient.patch()
			.uri(uri -> uri.path(PATH + "/" + ERRAND_ID + "/calculation/draft/header").build(Map.of("municipalityId", MUNICIPALITY_ID, "namespace", NAMESPACE)))
		...
		verify(calculationServiceMock).patchDraftHeader(eq(MUNICIPALITY_ID), eq(NAMESPACE), eq(ERRAND_ID), any(NormHeaderInput.class));
	}
