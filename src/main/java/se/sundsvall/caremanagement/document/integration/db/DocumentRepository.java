...
public interface DocumentRepository extends JpaRepository<DocumentEntity, String> {
	...
	/**
	 * Loads the document under a pessimistic write lock. Mutating paths (update/delete/lock) use this so a concurrent
	 * lock and edit are serialized: the second transaction blocks, then re-reads the now-{@code LOCKED} status and is
	 * rejected — closing the read-check-write race that would otherwise let an edit slip past skrivskydd.
	 */
	@Lock(LockModeType.PESSIMISTIC_WRITE)
	@Query("select e from DocumentEntity e where e.id = :id")
	Optional<DocumentEntity> findByIdForUpdate(@Param("id") String id);
}
