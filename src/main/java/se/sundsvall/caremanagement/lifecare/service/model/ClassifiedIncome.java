 ...
 * resolves the category to an FC income-type id and assembles the calculation; it no longer evaluates the regelverk.
 * Deserialised from the {@code classifiedIncomes} JSON the operaton {@code evaluate-income-regelverk} worker produces —
 * the JSON keys stay Swedish (the DMN contract), mapped onto English record components via {@link JsonProperty}.
 */
