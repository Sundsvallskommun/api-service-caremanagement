...
/**
 * Assembles the FC {@link PostAktualiseringsBodyRequest} for an ekonomiskt-bistånd intake (aktualisering) by resolving
 * the integer codes the POST body requires from the person's FC aktualisering proposal. The proposal's
 * {@code FromWho}/{@code Reason} code lists — and the {@code SpecifyType}/{@code WorkingStatus} requirement flags —
 * live
 * inside the chosen aktualisering <em>type</em>; the organisation, service and investigation links are top-level.
 *
 * <p>
 * Sprint defaults where the proposal offers a choice: the first offered aktualisering type is taken, then its first
 ...
 * for it — then the first offered value is used. {@code CaseworkerId} is deliberately left unset: the handläggare is
 * assigned later. These selections are intentionally simple and isolated here so they are easy to refine once real FC
 * proposals are available, mirroring {@link NormberakningAssembler}.
 */
public final class AktualiseringAssembler {

	private AktualiseringAssembler() {}

	/**
	 * Build the FC aktualisering body for one applicant and intake date.
	 *
	 * @param  applicantPersonId the applicant's personnummer (the FC aktualisering owner)
	 * @param  proposal          the FC aktualisering proposal supplying the code lists; may be {@code null}
	...
	/** The first offered aktualisering type — it carries the reason/fromWho code lists and the requirement flags. */
