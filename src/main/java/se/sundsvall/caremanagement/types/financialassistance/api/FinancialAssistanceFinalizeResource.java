...
class FinancialAssistanceFinalizeResource {
	...
	@Operation(summary = "Besluta och utbetala — finalize the errand (caseworker)",
		description = "Records the caseworker's decision as a PAYMENT decision on the errand, stores the chosen communication channels and household-size flag, hands the Lifecare write-backs to RPA (WRITE_DECISION, one REGISTER_PAYMENT per payment, and WRITE_MONITORING / WRITE_JOURNAL / WRITE_DOCUMENT for locally authored items), and correlates PaymentDecisionReceived to the process (paymentDecision APPROVED for BIFALL/DELAVSLAG, REJECTED otherwise) — which commits the normberäkning, sets GRANTED/REJECTED and polls the payment. Does not send anything to the applicant: the response echoes the communication channels for the frontend to act on. Requires the errand in AWAITING_DECISION with all three sections approved and an identified caller (X-Sent-By); a second finalize is rejected with 409. RPA and process failures are reported in the response, not as errors.",
		responses = {
			@ApiResponse(responseCode = "200", description = "Successful Operation", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "409", description = "Conflict - wrong status, sections not approved or already finalized", content = @Content(mediaType = APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = Problem.class)))
		})
